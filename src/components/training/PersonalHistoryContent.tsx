...
// The signed-in person's own training history (started, progress, score, certificates).
export async function PersonalHistoryContent({ userId }: { userId: string }) {
  ...
  const [{ data: progressRows }, { data: assignments }, { data: certificates }] = await Promise.all([
    ...
    supabase
      ...
      .eq('user_id', userId) as unknown as Promise<{ data: Pick<Assignment, 'module_id' | 'section_id'>[] | null }>,
    supabase
      .from('certificates')
      .select('*')
      .eq('user_id', userId)
